    ...
    <div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-x-auto">
            <table class="bg-white w-full border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tr v-for="category in categoriesData" :key="category.id" class="hover:bg-gray-50 transition">
                    <td class="p-4 border-b border-r border-gray-200">{{ category.id }}</td>
                    <td class="p-4 border-b border-r border-gray-200">{{ category.title }}</td>
                    <td class="p-4 border-b border-r border-gray-200">
                        <div class="flex justify-start gap-2">
                            <Link :href="route('admin.categories.show', category.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors" title="View">
                            ...
                            <Link :href="route('admin.categories.edit', category.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors" title="Edit">
                            ...
                            <button @click.prevent="deleteCategory(category)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-red-50 transition-colors" title="Delete">
    ...
    methods: {
        ...
        deleteCategory(category) {
            axios.delete(route('admin.categories.destroy', category.id))
                .then ( res => {
                    this.categoriesData = this.categoriesData.filter(categoryItem => categoryItem !== category)
                })
        }
    },
