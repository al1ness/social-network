<template>
    <div>
        <div class="mb-4 flex justify-center">
            <h1 class="text-2xl font-bold text-gray-800">Videos Management</h1>
        </div>
            ...
            <table class="bg-white w-full border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody class="text-gray-700 text-sm">
                    <tr v-for="video in videos" >
                        <td class="p-4 border-b border-r border-gray-200">{{ video.id }}</td>
                        <td class="p-4 border-b border-r border-gray-200">{{ video.title }}</td>
                        <td class="p-4 border-b border-r border-gray-200">{{ video.file_path }}</td>
                        <td class="p-4 border-b border-gray-200 text-center">
                            <div class="flex justify-center gap-2">
                                <Link :href="route('admin.videos.show', video.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors">
                                ...
                                <Link :href="route('admin.videos.edit', video.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors">
                                ...
                                <a href="#" @click.prevent="deleteVideo(video)" class="p-2 hover:bg-gray-100 rounded-lg">
...
export default {
    ...
    methods: {
        ...
        getVideos() {
            axios.get(route("admin.videos.index"), {
                params: this.filter
            })
                .then((res) => {
        ...
        deleteVideo(video) {
            axios.delete(route("admin.videos.destroy", video.id))
                .then(() => {
                    this.videosData = this.videosData.filter((videoItem) => videoItem.id !== video.id);
                });
        },
    },
