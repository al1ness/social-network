    ...
    <div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-x-auto">
            <table class="bg-white w-full border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tr v-for="tag in tagsData" :key="tag.id" class="hover:bg-gray-50 transition">
                    <td class="p-4 border-b border-r border-gray-200">{{ tag.id }}</td>
                    <td class="p-4 border-b border-r border-gray-200">{{ tag.title }}</td>
                    <td class="p-4 border-b border-gray-200 text-center">
                        <div class="flex justify-center gap-2">
                            <Link :href="route('admin.tags.show', tag.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors">
                            ...
                            <Link :href="route('admin.tags.edit', tag.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors">
                            ...
                            <a href="#" @click.prevent="deleteTag(tag)" class="p-2 hover:bg-gray-100 rounded-lg">
...
export default {
    ...
    methods: {
        ...
        getTags() {
            axios.get(route("admin.tags.index"), { params: this.filter })
                .then((res) => {
        ...
        deleteTag(tag) {
            axios.delete(route("admin.tags.destroy", tag.id))
                .then(() => {
                    this.tagsData = this.tagsData.filter((tagItem) => tagItem.id !== tag.id);
                });
        },
    },
