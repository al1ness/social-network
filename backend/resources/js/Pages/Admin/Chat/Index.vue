<template>
    <div>
        <div class="mb-4 flex justify-center">
            <h1 class="text-2xl font-bold text-gray-800">Chats Management</h1>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-x-auto">
            <table class="bg-white w-full border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tr v-for="chat in chatsData" :key="chat.id" class="hover:bg-gray-50 transition">
                    <td class="px-4 py-3 border-b border-r border-gray-200 font-medium">{{ chat.id }}</td>
                    <td class="px-4 py-3 border-b border-r border-gray-200">{{ chat.title }}</td>
                    <td class="px-4 py-3 border-b border-r border-gray-200">
                    ...
                    <td class="px-4 py-3 border-b border-gray-200">
                        <div class="flex justify-start gap-2">
                            <Link :href="route('admin.chats.show', chat.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors" title="View">
                            ...
                            <Link :href="route('admin.chats.edit', chat.id)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-sky-50 transition-colors" title="Edit">
                            ...
                            <button @click.prevent="deleteChat(chat)" class="inline-flex items-center justify-center p-2 rounded-md hover:bg-red-50 transition-colors" title="Delete">
...
export default {
    ...
    methods: {
        ...
        getChats() {
            axios.get(route('admin.chats.index'), {
                params: this.filter
            })
                .then( res => {
        ...
        deleteChat(chat) {
            if (confirm(`Are you sure you want to delete chat "${chat.title}"?`)) {
                axios.delete(route('admin.chats.destroy', chat.id))
                    .then( res => {
                        this.chatsData = this.chatsData.filter((chatItem) => chatItem.id !== chat.id)
                    })
            }
        }
    },
